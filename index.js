...
function createAuthorizationKey(EP_MerNo, web_key, params) {
  return crypto.createHash('md5')
    .update(EP_MerNo + web_key + params.EP_OrderNo + params.EP_Sum).digest('hex');
}

function Easypay(params, test) {
  this.url = test ? 'https://ssl.easypay.by/test/client_weborder.php' : 'https://ssl.easypay.by/weborder/';
  this.EP_MerNo = params.EP_MerNo;
  this.web_key = params.web_key;
}
...
Easypay.prototype.createInvoice = function(params) {
  ...
  var body = {
    ...
    'EP_Hash': createAuthorizationKey(this.EP_MerNo, this.web_key, params),
  ...
  request.post({
    ...
    if (err) d.reject(err);
    if (!body) d.reject(new Error('Body not found.'));
